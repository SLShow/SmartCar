...
#define LCD_SCL PTE1_OUT//PTA27_OUT  //K60的管脚配置//D2
#define LCD_SDA	PTE0_OUT//PTA26_OUT   //D0
#define LCD_RST PTE3_OUT//PTA25_OUT   //C6
#define LCD_DC  PTE2_OUT//PTA24_OUT //C18
//#define LCD_SCL PTC16_OUT//PTA27_OUT  //K60的管脚配置//D2
//#define LCD_SDA	PTC17_OUT//PTA26_OUT   //D0
//#define LCD_RST PTC18_OUT//PTA25_OUT   //C6
//#define LCD_DC  PTC19_OUT//PTA24_OUT //C18

/*
#define LCD_SCL PTB2_OUT//PTA27_OUT  //K60的管脚配置//D2
#define LCD_SDA	PTB3_OUT//PTA26_OUT   //D0
#define LCD_RST PTB0_OUT//PTA25_OUT   //C6
#define LCD_DC  PTB1_OUT//PTA24_OUT //C18
*/
...
void MAIN_ConvertData(uint16 *pInputImageData, uint8 *pOutputImageBuf)
{
 //   uint8 ucTmpData;
    ...
    for (i = 0; i < 128; i++)
    {
         temp[i]=(uint8)(pInputImageData[i]*60/256);
...
void MAIN_ConvertData1(uint16 *pInputImageData, uint8 *pOutputImageBuf)
{
 //   uint8 ucTmpData;
    uint8 i, usRowIdx;
    uint8 temp[128]={0};
...
//void dra_pixel3(void)
//{
//  char i;
//MAIN_ConvertData1(pixel3, *g_aucImageTable); //将数据转换为可以在OLED屏上显示的格式       
//LED_PrintImage(*g_aucImageTable, 60, 128); //在OLED上显示图像波形图
//        for(i=6;i<=7;i++)
//        {
//        LCD_Set_Pos(ccd3_left, i) ;     
//       LCD_WrDat(0xf0);
//        
//    
//        LCD_Set_Pos((int16)(ccd3_right/2+ccd3_left/2), i) ;
//        LCD_WrDat(0xfe);
//             
//        LCD_Set_Pos(ccd3_right, i) ;
//        LCD_WrDat(0x0f);
//        }
//}
//
...
//{
//  char i;
//MAIN_ConvertData1(pixel2, *g_aucImageTable); //将数据转换为可以在OLED屏上显示的格式       
//LED_PrintImage(*g_aucImageTable, 60, 128); //在OLED上显示图像波形图
//        for(i=6;i<=7;i++)
//        {
//        LCD_Set_Pos(ccd2_left, i) ;     
//        LCD_WrDat(0xf0);
//        
//    
//        LCD_Set_Pos((int16)(ccd2_right/2+ccd2_left/2), i) ;
//        LCD_WrDat(0xfe);
//             
//        LCD_Set_Pos(ccd2_right, i) ;
//        LCD_WrDat(0x0f);
//        }
//}
//uint16 pixel[128];
...
// uint16 i;
//
//MAIN_ConvertData1(pixel1, *g_aucImageTable); //将数据转换为可以在OLED屏上显示的格式       
//LED_PrintImage(*g_aucImageTable, 60, 128); //在OLED上显示图像波形图
//        for(i=6;i<=7;i++)
//        {
//        
//        LCD_Set_Pos(ccd1_left, i) ;     
//        LCD_WrDat(0xf0);
//        
//    
//        LCD_Set_Pos((int16)(ccd1_right/2+ccd1_left/2), i) ;
//        LCD_WrDat(0xfe);
//             
//        LCD_Set_Pos(ccd1_right, i) ;
//        LCD_WrDat(0x0f);
//        }
//}


/***************功能描述：显示8*16一组整型变量	显示的坐标（x,y），y为页范围0～7****************/
	...
	if(data<0)
	 {
	     ...
	     x+=6;   
            ...
            // LCD_write_number_6X8(x,y,(uint8)((int)data/1000%10));
             //x+=6;
	write_6_8_number(x,y,(uint8)((int)data/100%10));
	x+=6;
	write_6_8_number(x,y,(uint8)((int)data%100/10));
	x+=6;
	write_6_8_number(x,y,(uint8)((int)data%10));
	x+=6;      
}
...
void KeyScan (void)
{
  KeyRead(&g_KeyValue,&g_LongPressKeyValue);
  if(go_now)//启动
    {
	 LCD_DLY_ms(10);
     if(go_now)
	   {
        while(go_now);
       if(go_count==1)   go_count=0;    
       else go_count=1;
	   }
     if(go_count==1) LCD_CLS();
     else pre_show();
     
     }
  
  if(change_page_next) //下一页
    {
      LCD_DLY_ms(10);
     if(change_page_next) 
	     {
       while(change_page_next);
		   if(page_num<10)  
	            page_num++;
		   else
                   {
                     ...
                     line_num=0;
		    if(go_count==0) pre_show();
	     }  
     }

  
    if(change_page_last)//上一页 
    {
      LCD_DLY_ms(10);
     if(change_page_last) 
	     {
       while(change_page_last);
		   if(page_num>0)  
	            page_num--;
		   else
                   {
       ...
       while(change_line);
	        write_6_8_char(0,line_num,' ');  
	         ...
	         line_num++;
                   ...
                   {line_num=0;}
	       if(go_count==0)  write_6_8_char(0,line_num,'*');
	     } 	     
   }
...
if(Add_1)  
    {
     ...
     if(Add_1)
	   {
        while(Add_1);
      if(go_count==0)  change_value(page_num,line_num,1);  
	   }
    }
if(Sub_1) 
   {
